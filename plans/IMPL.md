...
[서버 :443 — Traefik]
   ...
   │
   ├── central-traefik (대시보드: traefik.{BASE_DOMAIN})
   ├── central-maintenance (점검 페이지, 항상 기동)
   ├── central-mysql
   ├── central-mysql-exporter
   ├── central-node-exporter
   ├── central-prometheus (central.{BASE_DOMAIN}/prometheus)
   ├── central-grafana (central.{BASE_DOMAIN}/grafana)
   ├── central-loki
   ├── central-promtail
   └── app-{name}-server ...
```
...
```
~/infra/.env          → BASE_DOMAIN, MYSQL_ROOT_PASSWORD, GF_SECURITY_ADMIN_PASSWORD, TRAEFIK_DASHBOARD_AUTH
...
| `~/infra/.env` | `BASE_DOMAIN`, 비밀번호들 | 앱 이름, 포트 |

**compose.attach.yml의 extends 경로**:
```yaml
services:
  app-server:
    extends:
...
./app upgrade myapp
   ...
   │
   ├─ 2. sleep 1 (Traefik 감지 대기)
   │
   ├─ 3. trap "rm -f maintenance-myapp.yml" EXIT (오류 시에도 복구 보장)
   │
   ├─ 4. docker compose pull + up -d --no-deps app-server
   │       구 컨테이너 종료 → 신 컨테이너 기동
   │       점검 페이지 계속 서빙
   │
   └─ 5. 스크립트 종료 시 trap에 의해 maintenance-myapp.yml 삭제
           Traefik 즉시 감지
...
services:
  app-server:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.${APP_NAME}.rule=Host(`${APP_NAME}.app.${BASE_DOMAIN}`)"
      - "traefik.http.routers.${APP_NAME}.entrypoints=websecure"
      - "traefik.http.routers.${APP_NAME}.tls=true"
      - "traefik.http.services.${APP_NAME}.loadbalancer.server.port=${APP_PORT}"
      - "traefik.http.routers.${APP_NAME}-http.rule=Host(`${APP_NAME}.app.${BASE_DOMAIN}`)"
      - "traefik.http.routers.${APP_NAME}-http.entrypoints=web"
      - "traefik.http.routers.${APP_NAME}-http.middlewares=redirect-to-https@file"

# Networks must be defined in the consumer compose file, not here.
# extends does not inherit top-level networks section.
```
...
```bash
docker compose \
  --env-file ~/infra/.env \       # BASE_DOMAIN, 비밀번호들 (인프라 전역)
...
| 역할 | 컨테이너 | 이미지 | 수집 대상 |
|---|---|---|---|
| 메트릭 수집 | central-prometheus | prom/prometheus:latest | Traefik, Node Exporter, MySQL Exporter, Spring Actuator |
| 호스트 메트릭 | central-node-exporter | prom/node-exporter:latest | CPU / 메모리 / 디스크 |
| MySQL 메트릭 | central-mysql-exporter | prom/mysqld-exporter:latest | MySQL 상태 |
| 로그 집계 | central-loki | grafana/loki:latest | Promtail이 전송한 로그 |
| 로그 수집 | central-promtail | grafana/promtail:latest | 전체 컨테이너 로그 (`docker.sock` 마운트) |
| 시각화 | central-grafana | grafana/grafana:latest | Prometheus + Loki 데이터소스 |
...
**Grafana 데이터소스 자동 구성 (datasources.yml)**:
```yaml
apiVersion: 1
...
|---|---|---|
| 리버스 프록시 | `central-traefik` | `traefik.${BASE_DOMAIN}` (대시보드) |
| 중앙 인프라 | `central-mysql`, `central-grafana`, `central-prometheus`, `central-loki`, `central-promtail` | `central.${BASE_DOMAIN}/grafana`, `central.${BASE_DOMAIN}/prometheus` |
| Exporter | `central-mysql-exporter`, `central-node-exporter` | 내부 전용 (Traefik 라우팅 없음) |
| 점검 페이지 | `central-maintenance` | 업그레이드 시에만 노출, 직접 접근 불가 |
| 각 앱 | `{app_name}-app-server-1` (Docker Compose 기본 네이밍) | `{name}.app.${BASE_DOMAIN}` |
...
```
~/infra/
├── .env                               # ★ BASE_DOMAIN, 모든 비밀번호 (도메인과 인증의 유일한 출처)
│                                      #   - BASE_DOMAIN
│                                      #   - MYSQL_ROOT_PASSWORD
...
│
├── central-infra/
│   ├── compose.yml                    # Traefik, MySQL, Grafana, Prometheus, Loki, Maintenance, Exporters
│   ├── traefik/
│   │   ├── traefik.yml                # static config (entrypoints, provider 설정)
│   │   ├── dynamic/                   # ★ 파일 프로바이더 감시 디렉터리 (watch: true)
│   │   │   ├── dashboard-auth.yml     # ./infra start가 TRAEFIK_DASHBOARD_AUTH에서 생성
│   │   │   └── (평시 maintenance-*.yml 없음)
│   │   │       maintenance-{name}.yml # upgrade 스크립트가 생성/삭제
│   │   └── certs/
│   │       ├── origin.pem             # Cloudflare Origin Certificate (수동 배치)
│   │       └── origin.key
│   ├── prometheus/
│   │   └── prometheus.yml             # scrape targets + Docker SD 설정
│   ├── loki/
│   │   └── loki-config.yml            # 로그 집계 설정
│   ├── promtail/
│   │   └── promtail-config.yml        # 로그 수집 설정 (Docker SD)
│   ├── grafana/
│   │   └── provisioning/
│   │       └── datasources/
│   │           └── datasources.yml    # Prometheus, Loki 자동 구성
│   ├── maintenance/
...
│       └── compose.attach.yml
│
└── test-scenario.sh                   # PRD 기반 통합 테스트 스크립트
```
...
./infra setup      # 최초 환경 구성 — 멱등적 실행 보장
                   ...
                   # 전제조건 체크: Docker 설치, 데몬 실행, 포트 80/443 사용 가능 여부

./infra start      # 중앙 인프라 기동 → apps/ 하위 전체 앱 순서대로 기동
                   # 1. validate_env_vars: 필수 환경변수 확인
                   #    - BASE_DOMAIN
                   #    - MYSQL_ROOT_PASSWORD
                   #    - GF_SECURITY_ADMIN_PASSWORD
                   #    - TRAEFIK_DASHBOARD_AUTH
                   # 2. generate_dashboard_auth: TRAEFIK_DASHBOARD_AUTH에서 dashboard-auth.yml 생성
                   # 3. central-infra/compose.yml 기동
                   # 4. apps/ 디렉터리의 모든 앱 순차 기동

./infra stop       # 앱 역순 종료 → 중앙 인프라 종료 (볼륨 삭제 없음)
./infra status     # 모든 컨테이너 상태 일람 출력 (프로젝트별 그룹화)
./infra archive    # 마이그레이션용 아카이브 생성
                   # 확인 프롬프트 → ./infra stop → tar → 파일 경로 및 크기 안내
```

**TRAEFIK_DASHBOARD_AUTH 생성 방법**:
```bash
htpasswd -nbB admin <your-password> | sed 's/\$/\$\$/g'
# 출력 예시: admin:$$2y$$10$$xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
upgrade() {
  ...
  # 2. 업그레이드 (set -e: 실패 시 즉시 중단, trap에서 파일 삭제 보장)
  trap "rm -f $MAINTENANCE_FILE" EXIT
  docker compose $COMPOSE_ARGS pull
  docker compose $COMPOSE_ARGS up -d --no-deps app-server

  # 3. 라우팅 복구 (trap에 의해 보장)
}
```

**maintenance-{name}.yml 생성 내용**:
```yaml
http:
  routers:
    maintenance-myapp:
      rule: "Host(`myapp.app.example.com`)"
      entrypoints:
        - websecure
      tls: true
...
| 메트릭/로그 모니터링 | Prometheus + Grafana + Loki + Promtail + Node/MySQL Exporter |
...
### 12-1. HTTP→HTTPS 리다이렉트 미들웨어

`app-attach-base.yml`에서 `redirect-to-https@file` 미들웨어를 참조하지만, 현재 `traefik/dynamic/` 디렉터리에 해당 미들웨어 정의가 없다.

**현재 상태**: Traefik 정적 설정의 entrypoint 레벨 리다이렉트가 작동하므로 기능적으로는 문제없음

**향후 개선**: 명시적인 미들웨어 정의 추가 권장
```yaml
# traefik/dynamic/redirect.yml
http:
  middlewares:
    redirect-to-https:
      redirectScheme:
...
`test-scenario.sh`가 PRD 요구사항 기반 통합 테스트를 제공한다:
- 인프라 설정 및 기동
- 앱 생성, 시작, 중지
- 동적 앱 추가/제거 (무중단)
- 브라우저 URL 검증
